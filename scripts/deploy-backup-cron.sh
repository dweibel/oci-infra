...
# Deploy Wiki.js backup cron job to the OCI instance.
#
# Prerequisites:
...
BACKUP_SCRIPT="/home/opc/scripts/backup-wikijs.sh"
CRON_TAG="# wikijs-backup"
LOG_FILE="/mnt/workspace/backups/wikijs/backup.log"
...
install_local() {
  ...
  if [ -f "${SCRIPT_DIR}/backup-wikijs.sh" ]; then
    cp "${SCRIPT_DIR}/backup-wikijs.sh" "$BACKUP_SCRIPT"
  ...
  local env_vars="WIKIJS_BACKUP_BUCKET=${OCI_BACKUP_BUCKET}"
  local cron_line="${SCHEDULE} ${env_vars} ${BACKUP_SCRIPT} >> ${LOG_FILE} 2>&1 ${CRON_TAG}"
...
if [ -n "$REMOTE_HOST" ]; then
  ...
  ssh "$REMOTE_HOST" "
    ...
    mkdir -p $(dirname "$BACKUP_SCRIPT") $(dirname "$LOG_FILE")
    cp /tmp/backup-wikijs.sh ${BACKUP_SCRIPT}
    chmod +x ${BACKUP_SCRIPT}
    ...
    if [ \"\${REMOVE}\" = 'true' ]; then
      ...
      CRON_LINE='${SCHEDULE} WIKIJS_BACKUP_BUCKET=${OCI_BACKUP_BUCKET} ${BACKUP_SCRIPT} >> ${LOG_FILE} 2>&1 ${CRON_TAG}'
      (crontab -l 2>/dev/null | grep -v '${CRON_TAG}'; echo \"\${CRON_LINE}\") | crontab -
      echo ''
